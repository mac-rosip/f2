...
class Dispatcher {
	...
	public:
		Dispatcher(cl_context & clContext, cl_program & clProgram, const Mode mode, const size_t worksizeMax, const size_t inverseSize, const size_t inverseMultiple, const cl_uchar clScoreQuit, const std::string & seedPublicKey, const std::string & contractAddress = "", const std::string & senderAddress = "");
		~Dispatcher();
