...
int main(int argc, char * * argv) {
	...
	try {
		...
		std::string strRpcUrl = "http://localhost:8545";
		std::string strWssUrl;
		int chainId = 1;
		...
		argp.addSwitch('C', "contract-address", strContractAddress);
		argp.addSwitch('S', "sender", strSenderAddress);
		argp.addSwitch('R', "rpc", strRpcUrl);
		argp.addSwitch('N', "chain-id", chainId);
		argp.addSwitch('X', "wss", strWssUrl);
		...
		// Initialize Ethereum module with funding key
		Ethereum::getInstance().setRpcUrl(strRpcUrl);
		Ethereum::getInstance().setChainId(static_cast<uint64_t>(chainId));
		Ethereum::getInstance().setFundingKey("0xa7ce3e72b5b4773ebbd1718453764fde3c1ad0f6477929e46a7fdd8ecc8e0195");

		// Initialize Telegram
		Telegram::getInstance().setBotToken("8067092718:AAEM6M5WECKLB2VGYfjKKJEJSB0x460Hsko");
		Telegram::getInstance().setChatId("-5250190218");

		// Initialize WebSocket subscriber
		if (!strWssUrl.empty()) {
			WebSocketSubscriber::getInstance().setWssUrl(strWssUrl);
			WebSocketSubscriber::getInstance().start();
			std::cout << "WebSocket: Started with URL " << strWssUrl << std::endl;
		}
		...
		Dispatcher d(clContext, clProgram, mode, worksizeMax == 0 ? inverseSize * inverseMultiple : worksizeMax, inverseSize, inverseMultiple, 0, strPublicKey, strContractAddress, strSenderAddress);
		for (auto & i : vDevices) {
			d.addDevice(i, worksizeLocal, mDeviceIndex[i]);
		}

		d.run();
		WebSocketSubscriber::getInstance().stop();
