...
Dispatcher::Dispatcher(cl_context & clContext, cl_program & clProgram, const Mode mode, const size_t worksizeMax, const size_t inverseSize, const size_t inverseMultiple, const cl_uchar clScoreQuit, const std::string & seedPublicKey, const std::string & contractAddress, const std::string & senderAddress)
	...
	for (auto i = PROFANITY_MAX_SCORE; i > m_clScoreMax; --i) {
		...
		if (r.found > 0 && i >= d.m_clScoreMax) {
			...
			std::lock_guard<std::mutex> lock(m_mutex);
				...
				// Compute private key offset and derived address for storage
				cl_ulong carry = 0;
				cl_ulong4 seedRes;
				seedRes.s[0] = d.m_clSeed.s[0] + d.m_round; carry = seedRes.s[0] < d.m_round;
				seedRes.s[1] = d.m_clSeed.s[1] + carry; carry = !seedRes.s[1];
				seedRes.s[2] = d.m_clSeed.s[2] + carry; carry = !seedRes.s[2];
				seedRes.s[3] = d.m_clSeed.s[3] + carry + r.foundId;

				std::ostringstream ssPrivate;
				ssPrivate << std::hex << std::setfill('0');
				ssPrivate << std::setw(16) << seedRes.s[3] << std::setw(16) << seedRes.s[2] << std::setw(16) << seedRes.s[1] << std::setw(16) << seedRes.s[0];
				const std::string strPrivateKey = "0x" + ssPrivate.str();
				...
				WebSocketSubscriber::getInstance().subscribeToAddress(strDerivedAddress);

				// Note: Telegram notifications are only sent for WSS updates, not for initial vanity address discovery
				
				// Now that all transactions are complete, set quit flag if needed
